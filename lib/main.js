...
export default class AtomDebugger {
  ...
  constructor() {
    ...
    this.subscriptions.add(this.uiRegistry)
    ...
    this.commands.onDidStart(delegate => {
      this.uiRegistry.didStart(delegate)
    })
    this.commands.onDidStop(delegate => {
      this.uiRegistry.didStop(delegate)
    })
  }
  activate() {
