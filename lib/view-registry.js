...
import type { DebuggerView, Notification } from './types'
...
export default class ViewRegistry {
  ...
  isRunning(notification: Notification) {
    this.providers.forEach(function(provider) {
      provider.isRunning(notification)
    })
  }

  stoppedExecution(notification: Notification) {
    this.providers.forEach(function(provider) {
      provider.stoppedExecution(notification)
    })
  }

  triggeredBreakpoint(notification: Notification) {
    this.providers.forEach(function(provider) {
      provider.triggeredBreakpoint(notification)
    })
  }
}
